...
import pylab, string, random, sys

#sys.path.append('C:\Users\Gurin\Canopy\edx\MITx6.00.2x\ProblemSet6')
#sys.path.remove('C:\Users\Gurin\Canopy\edx\MITx6.00.2x\ProblemSet6')
#print sys.path
...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        for point in self.members():
            for otherPoint in other.members():
    ...
    def averageLinkageDist(self, other):
        ...
        for point in self.members(): 
            for otherPoint in other.members():
                result += point.distance(otherPoint)
                length += 1
...
class ClusterSet(object):
    ...
    def findClosest(self, linkage):
        ...
        for cluster in self.members:
            for otherCluster in self.members:
                if best == None and cluster != otherCluster:
                    best = linkage(cluster, otherCluster)
                    result = (cluster, otherCluster)
                elif cluster != otherCluster:
                    if linkage(cluster, otherCluster) <= best:
                        best = linkage(cluster, otherCluster)
                        result = (cluster, otherCluster)
        return result
...
#pSet1 = [Point(str(i), [i, i]) for i in range(2)]
#pSet2 = [Point(str(i), [i, i]) for i in range(1,2)]
#C1 = Cluster(pSet1, type(pSet1[0]))
#C2 = Cluster(pSet2, type(pSet2[0]))
#
#print C1
#print C2
...
def test():
    ...
    #C1 = Cluster(points[:len(points)/2], type(points[0]))
    #C2 = Cluster(points[len(points)/2:], type(points[0]))
    #print C1
    #print C1.singleLinkageDist(C2)
    #print C1.maxLinkageDist(C2)
    #print C1.averageLinkageDist(C2)
